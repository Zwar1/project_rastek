...
const CalendarMainLayer = () => {
  const { CalendarEvents, loading, error, refreshEvents,
    addCompanyCalendar, editCompanyCalendar, deleteCompanyCalendar } = useCalendar();
  ...
  const handleLeaveSave = async (e) => {
    ...
    try {
      ...
      alert("Leave event successfully created!");

      // Dismiss the modal using the hidden dismiss button
      document.getElementById("modalDismissButton").click();
      ...
      // Reset leave form: pre-populate with current date and isFree true
  ...
  const handleSaveEdit = async (e) => {
    e.preventDefault();
    console.log('Current editEventData:', editEventData);

    const now = new Date();
    now.setHours(0, 0, 0, 0);

    if (!editEventData.startDate || !editEventData.eventName) {
      alert("Please fill all required fields");
      return;
    }

    if (new Date(editEventData.startDate) < now) {
      alert("Start date cannot be in the past");
      return;
    }

    if (new Date(editEventData.endDate) < new Date(editEventData.startDate)) {
                  ...
                  [...CalendarEvents].sort((a, b) => {
                    ...
                    <div key={event.id} className="event-item d-flex align-items-center justify-content-between gap-4 pb-16 mb-16 border border-start-0 border-end-0 border-top-0">
                      <div className="">
                        ...
                        <span className="text-primary-light fw-semibold text-md mt-4 " style={{
                          wordWrap: 'break-word',
                          maxWidth: '200px'
                        }}>
                          {event.eventName}
                    ...
                    <div className="form-check d-flex align-items-center gap-2">
                      <input
                        ...
                        onChange={(e) => setFormData(prev => ({
                          ...prev,
                          isFree: e.target.value === 'holiday'
                        }))}
                        id="holidayStatus"
                        className="form-check-input"
                      />
                      <label
                        htmlFor="holidayStatus"
          ...
          <div className="modal-content radius-16 bg-base">
            ...
            <div className="modal-body p-24">
              ...
              <div className="mb-12">
                ...
                <h6 className="text-primary-light fw-semibold text-md mb-0 mt-4">
                  {viewEvent?.startDate ? formatDate(viewEvent.startDate) : "N/A"}
                </h6>
              </div>
              <div className="mb-12">
                <span className="text-secondary-light txt-sm fw-medium">
                ...
                <h6 className="text-primary-light fw-semibold text-md mb-0 mt-4">
                  {viewEvent?.endDate ? formatDate(viewEvent.endDate) : "N/A"}
                </h6>
              </div>
              <div className="mb-12">
                <span className="text-secondary-light txt-sm fw-medium">
              ...
              <div className="mb-12">
                ...
                <h6 className="text-primary-light fw-semibold text-md mb-0 mt-4 d-flex align-items-center gap-2">
                  <span className={`w-8-px h-8-px ${viewEvent?.isFree ? 'bg-success-600' : 'bg-warning-600'} rounded-circle`}
                  />
                  {viewEvent?.isFree ? "Holiday" : "Work"}
                </h6>
              </div>
            </div>
          </div>
                  ...
                  <div className="col-md-6 mb-20">
                    ...
                    <div className=" position-relative">
                      <DatePicker
                        ...
                        onChange={(date) => setEditEventData(prev => ({
                          ...prev,
                          startDate: date
                        }))}
                        minDate={new Date()}
