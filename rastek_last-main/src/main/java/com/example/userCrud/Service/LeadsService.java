...
public class LeadsService {
    ...
    public void deleteLead(Long id) {
        LeadsEntity leadsEntity = leadsRepository.findFirstById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Lead not found"));

        if ("Accepted".equals(leadsEntity.getStatus())) {
            ClientEntity clientEntity = clientRepository.findByClientName(leadsEntity.getClientName());

            // Check if client exists before trying to delete
            if (clientEntity != null) {
                // Check and delete associated user if exists
                if (clientEntity.getUser() != null) {
                    userRepository.delete(clientEntity.getUser());
                }
                clientRepository.delete(clientEntity);
            }
        }

        // Delete the lead regardless of client status
